...
import {X, H, CX, SWAP, CSWAP} from "../../../QCompute/QPlatform/Operation/FixedGate";
import {U, CU} from "../../../QCompute/QPlatform/Operation/RotationGate";
...
function CU1(q1, q2, float_theta)
{
    /*
...
function func_qftadd_inverse(reg_system, int_adder)
{
    /*
    ...
    for(let idx_qubit = 0; idx_qubit < reg_system.length; idx_qubit++)  // For each qubit in the reg_s, we operate a U1 gate on it
    {
        U(-2 * pi * (int_adder % 2 ** (idx_qubit + 1)) / (2 ** (idx_qubit + 1)))(
            reg_system[reg_system.length -1 - idx_qubit]);
    }
}


function func_ctrl_qftadd(reg_system, qubit_ctrlling, int_adder)
{
     ...
     */
    for(let idx_qubit = 0; idx_qubit < reg_system.length; idx_qubit++)  // For each qubit in the reg_s, we operate a CU1 gate on |c> and it
    {
        CU1(qubit_ctrlling,reg_system[reg_system.length -1 - idx_qubit],
            2 * pi * math.mod(int_adder, 2 ** (idx_qubit + 1)) / (2 ** (idx_qubit + 1)));
    }
}


function func_ctrl_qftadd_inverse(reg_system, qubit_ctrlling, int_adder)
{
    /*
...
function func_double_ctrl_qftadd(reg_system, reg_ctrlling, int_adder)
{
    /*
     ...
     */
    for(let idx_qubit = 0; idx_qubit < reg_system.length; idx_qubit++)  // For each qubit in the reg_s, we operate a CCU1 gate on |c_1>|c_2> and it
    {
        CCU1(reg_ctrlling[0], reg_ctrlling[1], reg_system[reg_system.length -1 - idx_qubit],
            2 * pi * math.mod(int_adder, 2 ** (idx_qubit + 1)) / (2 ** (idx_qubit + 1)));
    }
}


function func_double_ctrl_qftadd_inverse(reg_system, reg_ctrlling, int_adder)
{
    /*
...
function func_swap_in_qft(reg_system)
{
    /*
    ...
    for(let idx_qubit = 0; idx_qubit < Math.floor(number_qubit / 2); idx_qubit++)  // SWAP the i-th qubit and the (n-i-1)-th qubit
    {
...
function func_qft_without_swap_inverse(reg_system)
{
    /*
    ...
    for(let idx1_qubit = 0; idx1_qubit < number_qubit - 1; idx1_qubit++)  // The outer loop
    {
        ...
        for(let idx2_qubit = 0; idx2_qubit < idx1_qubit + 1; idx2_qubit++)  // The inner loop where we will operate a CU1 gate in each loop
        {
...
function func_double_ctrl_qftaddmod_inverse(reg_system, reg_ctrlling, qubit_zeroed, int_adder, int_divisor)
{
    /*
...
function func_ctrl_addprodmod_inverse(reg_system, reg_factor_1, qubit_ctrlling, qubit_zeroed, int_factor_2, int_divisor)
{
    /*
    ...
    for(let idx_qubit = 0; idx_qubit < reg_factor_1.length; idx_qubit++)  // For each qubit in reg_f_1, we will operate a CC-qftaddmod gate where
        // regarding idx_qubit as one of the two ctrlling qubit
        func_double_ctrl_qftaddmod_inverse(reg_system, [qubit_ctrlling, reg_factor_1[reg_factor_1.length -1 - idx_qubit]], qubit_zeroed,
            (int_factor_2 * (2 ** idx_qubit)) % int_divisor, int_divisor);
    func_qft_without_swap_inverse(reg_system);
}
...
function func_from_fraction_continued(list_int_generator)
{
     ...
     */
    let int_denominator = 1;
    let int_numerator = 0;
    while(list_int_generator.length != 0)  // We use a stack data structure to compute the denominator of continued
    {   // fraction, and the fraction is stored as two int: the denominator and the numerator
        let int_denominator_new = list_int_generator.pop() * int_denominator + int_numerator;
...
function func_result_to_order(int_numerator, number_qubit, int_max_denominator)
{
    /*
     ...
     */
    let list_int_generator_fraction = [];
    let int_denominator = 2 ** number_qubit;
    while(int_denominator != 0)  // By Euclidean algorithm we compute the representation in continued fraction
    {
        list_int_generator_fraction = list_int_generator_fraction.concat(Math.floor(int_numerator / int_denominator));
        let int_remainder = int_numerator % int_denominator;
        int_numerator = int_denominator;
        int_denominator = int_remainder;
    }
    // Thus the coefficients form list_int_gen._frac.
    for(let idx_int = 0; idx_int < list_int_generator_fraction.length; idx_int++)  // From front to back, from short to long,
    {   // we truncate the continued fraction
        if (func_from_fraction_continued(list_int_generator_fraction.slice(0, idx_int + 1)) >= int_max_denominator)
            return func_from_fraction_continued(list_int_generator_fraction.slice(0, idx_int));
    }
    return func_from_fraction_continued(list_int_generator_fraction);
}


function func_result_to_order_list(int_numerator, number_qubit, int_max_denominator)
{
    /*
    ...
    for(let idx_int = 0; idx_int < list_int_generator_fraction.length; idx_int++)  // From front to back, from short to long,
    {   // we truncate the continued fraction
        let int_order_maybe = func_from_fraction_continued(list_int_generator_fraction.slice(0, idx_int + 1));
        if (int_order_maybe < int_max_denominator)
            list_int_order = list_int_order.concat(int_order_maybe);
        else
            break;
    }
